...
column_names = ['game_id', 'game_name', 'publisher', 'genre', 'description']

# Load data from the file and set the column names
user_data = pd.read_csv('CSV/Games.csv', header=None, names=column_names)
...
# Generate the SQL INSERT statement for the Games table
user_insert_sql = generate_insert_statements("Games", user_data)

# Print the SQL statement to the console
print(user_insert_sql)

# Save the SQL statement to a file
with open('insert_game.sql', 'w', encoding='utf-8') as user_file:
